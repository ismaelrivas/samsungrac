...
class Samsung8888Client:
    ...
    async def _create_ssl_context(self) -> ssl.SSLContext:
        """Configure SSL, replicating the logic from connection_request.py and tls_auto."""
        if self.ssl_mode == "modern":
            # Equivalent to connection_request_tls_auto.py
            ctx = ssl.create_default_context()
        else:
            # Equivalent to connection_request.py (Legacy)
    ...
    async def connect(self):
        ...
        try:
            ...
            raise ConnectionError(f"Connection error [{self.ssl_mode}]: {e}")
        ...
        async with self._lock:
            ...
            while retry:
                ...
                try:
                    ...
                    _LOGGER.debug("%s Response body received: '%s'", self.log_prefix, resp_body.replace("\r", "").replace("\n", ""))
