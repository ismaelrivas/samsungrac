...
class ConnectionRequestBase(Connection):
    def __init__(self, hass_config, _logger, insecure_ssl=False, timeout=5, retry_delay=1.0, debug=False):
    ...
    def execute_internal(self, template, value, device_state, device_id=None) -> (json, bool, int):
        ...
        with warnings.catch_warnings():
            ...
            with requests.sessions.Session() as session:
                ...
                if self._debug:
                    _LOGGER.debug("%s Executing request with params: %s", self.log_prefix, _mask_request_params(params, self.log_prefix))
