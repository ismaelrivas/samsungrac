...
class SamsungTokenAcquirer8888:
        ...
        class TokenHandler(http.server.BaseHTTPRequestHandler):
            ...
            def do_POST(self):
                try:
                    ...
                    _LOGGER.error(f"Error handling POST request from AC: {e}", exc_info=True)
    ...
    def _start_listener_server(self):
        ...
        try:
            ...
            _LOGGER.info(f"Temporary token listener server started on port {self._listener_port}.")
            return True
        except Exception as e:
            _LOGGER.error(f"Failed to start listener server: {e}", exc_info=True)
            return False
    ...
    async def async_initiate_pairing(self) -> None:
        ...
        _LOGGER.info(f"Requesting token from AC at {self._ac_ip}:{self._ac_port}")
