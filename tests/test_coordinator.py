...
async def test_coordinator_strike_1_and_2_return_stale_data(hass: HomeAssistant) -> None:
    ...
    mock_state_getter = MagicMock()
    mock_state_getter.async_update_state = AsyncMock(
        side_effect=[{"power": "on"}, CannotConnect("Timeout"), CannotConnect("Connection refused")]
    )
    mock_controller.loader.state_getter = mock_state_getter
    mock_controller.poller = YamlStatePoller(mock_controller)
...
async def test_coordinator_handles_503_transient_smartthings(hass: HomeAssistant) -> None:
...
async def test_save_new_token_updates_config_entry(hass: HomeAssistant) -> None:
    ...
    with patch.object(hass.config_entries, "async_update_entry") as mock_update_entry, \
        ...
        mock_update_entry.assert_called_once_with(
            mock_entry,
            data={"host": "192.168.1.10", "token": "new_token_2878"}
        )
        mock_info.assert_called_once_with("%s Persisted new network token to Config Entry.", "[TokenTest]")
...
async def test_save_new_token_async_flow(hass: HomeAssistant) -> None:
    ...
    mock_update_entry.assert_called_once_with(
        mock_entry,
        data={"host": "192.168.1.10", "token": "new_token_async"}
    )
...
async def test_coordinator_injected_callbacks(hass: HomeAssistant) -> None:
    ...
    assert mock_controller.on_push_update_callback == coordinator.async_handle_push_update
...
async def test_save_ssl_config_updates_entry(hass: HomeAssistant) -> None:
    ...
    with patch.object(hass.config_entries, "async_update_entry") as mock_update_entry, \
        ...
        mock_info.assert_called_once_with("%s Persisted SSL config to ConfigEntry data.", "[SSLTest]")
    ...
    with patch.object(hass.config_entries, "async_update_entry") as mock_update_entry_2:
        mock_entry.data = {"host": "192.168.1.10", "_ssl_config_2878": {"cert": "new_cert"}}
        mock_controller.on_ssl_config_updated({"cert": "new_cert"})
        mock_update_entry_2.assert_not_called()


async def test_coordinator_update_interval_enable_polling_options(hass: HomeAssistant) -> None:
...
async def test_coordinator_device_info_complete_attributes(hass: HomeAssistant) -> None:
    ...
    coordinator = SamsungClimateCoordinator(
        hass, mock_controller, mock_entry, device_info=device_info, parent_unique_id="parent_mac_123"
    )
...
async def test_coordinator_standalone_device_info(hass: HomeAssistant) -> None:
    ...
    assert coordinator.device_info["manufacturer"] == "Samsung"
    assert coordinator.device_info["connections"] == {(dr.CONNECTION_NETWORK_MAC, "00:11:22:33:44:55")}

async def test_coordinator_standalone_device_info_no_mac_no_name(hass: HomeAssistant) -> None:
...
async def test_fallback_to_raw_engine_on_http_header_error(hass: HomeAssistant) -> None:
    ...
    mock_state_getter = MagicMock()
    mock_state_getter.async_update_state = AsyncMock(side_effect=InvalidHeaderError("Malformed headers"))
    mock_controller.loader.state_getter = mock_state_getter
    mock_controller.poller = YamlStatePoller(mock_controller)
    mock_controller.async_get_status = mock_controller.poller.async_update_state
...
async def test_coordinator_update_interval_enable_polling_data(hass: HomeAssistant) -> None:
...
async def test_coordinator_update_interval_enable_polling_default(hass: HomeAssistant) -> None:
...
async def test_coordinator_update_interval_poll_interval_options(hass: HomeAssistant) -> None:
...
async def test_coordinator_update_interval_poll_interval_data(hass: HomeAssistant) -> None:
...
async def test_coordinator_update_interval_poll_interval_default(hass: HomeAssistant) -> None:
...
async def test_coordinator_update_interval_enable_polling_data_false(hass: HomeAssistant) -> None:
...
async def test_async_set_property_passes_device_id(hass: HomeAssistant) -> None:
    ...
    await coordinator.async_set_property("hvac_mode", "dry", device_id="child_device_123")

    mock_controller.async_set_property.assert_awaited_once_with("hvac_mode", "dry", "child_device_123")

async def test_async_set_property_raises_update_failed_on_exception_with_message(hass: HomeAssistant) -> None:
    ...
    mock_controller.async_set_property = AsyncMock(side_effect=Exception("Simulated unexpected failure"))
    mock_controller.climate_state = MagicMock(hvac_mode="cool")
    ...
    with pytest.raises(UpdateFailed, match="Failed to set property hvac_mode: Simulated unexpected failure"):
        await coordinator.async_set_property("hvac_mode", "dry")
...
async def test_coordinator_enforces_strict_timeout(hass: HomeAssistant) -> None:
    ...
    with patch("custom_components.climate_ip.coordinator.asyncio.wait_for") as mock_wait:
        await coordinator._async_update_data()
...
async def test_coordinator_unwraps_hvac_enum_before_sending(hass: HomeAssistant) -> None:
    ...
    mock_controller.async_set_property.assert_awaited_once_with("hvac_mode", "cool", None)

async def test_coordinator_requests_refresh_on_partial_failure(hass: HomeAssistant) -> None:
