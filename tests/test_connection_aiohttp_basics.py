...
async def test_get_session_args():
    ...
    with patch(f"{_NS}.TCPConnector") as mock_connector, \
...
def test_format_url_strict_defaults_and_http_downgrade():
    ...
    assert formatted.startswith("https://"), "Mutante alteró el default de use_http (False)"
    ...
    assert formatted_http.startswith("http://"), "Mutante rompió el downgrade a HTTP plano"
...
async def test_get_session_recreates_on_closed_session():
    ...
    with patch(f"{_NS}.TCPConnector") as mock_connector, \
        ...
        assert conn._shared_state.local_session is fresh_session, "No reemplazó la sesión cerrada"
...
async def test_get_session_fallback_when_shared_session_is_none():
    ...
    with patch(f"{_NS}.TCPConnector"), \
        ...
        mock_session_cls.assert_called_once()
        assert result == "fallback_local_session_obj", "El fallback a sesión local falló"
...
@pytest.mark.asyncio
async def test_embedded_command_without_template_uses_params_directly(mock_session, mock_logger, mock_hass):
...
@pytest.mark.asyncio
async def test_embedded_command_without_template_and_empty_params_skips(mock_session, mock_logger, mock_hass):
...
@pytest.mark.asyncio
async def test_embedded_command_method_and_url_fallback_to_main(mock_session, mock_logger, mock_hass):
...
@pytest.mark.asyncio
async def test_embedded_command_strict_fallbacks_to_main(mock_session, mock_logger, mock_hass):
    ...
    await conn.async_execute("PUT", "/main_url", "main_data_ignored", main_headers, device_state={"state": "on"})
...
@pytest.mark.asyncio
@pytest.mark.parametrize("is_poll,keep_alive,should_close", [
...
async def test_try_connection_strict_http_mode(mock_logger, mock_hass):
    ...
    mock_local_session.closed = False  # CRÍTICO: evita que _get_session intente crear una nueva
    mock_response = AsyncMock()
    mock_response.status = 200
    ...
    with patch.object(conn, "_create_ssl_context", new_callable=AsyncMock) as mock_ssl_create:
...
@pytest.mark.asyncio
@pytest.mark.parametrize("major,minor,start_forced,expected_forced", [
...
@pytest.mark.asyncio
async def test_async_execute_request_strict_kwargs(mock_session, mock_logger, mock_hass):
    ...
    await conn._async_execute_request("PATCH", "/strict_url", "strict_payload", {"Header": "Strict"})
...
@pytest.mark.asyncio
async def test_async_execute_request_none_http_version(mock_session, mock_logger, mock_hass):
    ...
    assert conn._force_close_connection is True, "Debería forzar cierre si no hay versión"
    # Verificamos que el logger.debug NO se llamó con el mensaje de versión
    for call in mock_logger.debug.call_args_list:
        assert "Server speaks HTTP" not in call[0][0], "El mutante evaluó True en `and response.version` cuando era None"
...
async def test_async_execute_request_absolute_url(mock_session, mock_logger, mock_hass):
    ...
    assert kwargs["url"] == absolute_url, f"El mutante rompió startswith('http'): {kwargs['url']}"
...
async def test_try_connection_strict_error_handling(mock_logger, mock_hass, status_code):
    ...
    """
    conn = ConnectionAiohttp8888(
        config={"token": "tok"}, logger=mock_logger, hass=mock_hass, session=None, ip_address="1.1.1.1"
    )
...
@pytest.mark.asyncio
async def test_try_connection_strict_token_and_port(mock_session, mock_logger, mock_hass):
    ...
    conn = ConnectionAiohttp8888(
        ...
        logger=mock_logger, hass=mock_hass, session=None, ip_address="1.1.1.1"
    )
    ...
    # ASERCIONES MILIMÉTRICAS
    assert kwargs["headers"]["Authorization"] == "Bearer CTRL_TOK", "Mutante rompió fallback del controller token"
    assert args[1] == "https://1.1.1.1:7777/devices", "Mutante rompió el puerto en _try_connection"

@pytest.mark.asyncio
async def test_async_execute_request_header_placeholders(mock_session, mock_logger, mock_hass):
    """Mata mutantes de format_placeholders aplicados a las cabeceras."""
    conn = ConnectionAiohttp8888(
        config={"token": "tok"},
        logger=mock_logger, hass=mock_hass, session=mock_session, ip_address="1.1.1.1"
    )
    ...
    assert kwargs["headers"]["X-Dev"] == "DEV_123", "Mutante rompió placeholder {device_id}"
    assert kwargs["headers"]["X-Tok"] == "tok", "Mutante rompió placeholder {token}"

@pytest.mark.asyncio
async def test_async_execute_skips_optimization_on_mismatch(mock_session, mock_logger, mock_hass):
    ...
    with patch.object(conn, "_try_connection", new_callable=AsyncMock) as mock_try, \
         patch.object(conn, "_async_execute_request", new_callable=AsyncMock) as mock_req:
        ...
        assert res1 == "REQ_OK", "El mutante activó la optimización erróneamente para POST"
