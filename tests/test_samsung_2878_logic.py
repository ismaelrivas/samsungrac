...
def test_samsung_2878_properties_and_setters(connection):
    ...
    assert connection.is_async_native != False  # Kill boolean mutants
    assert connection.is_push_supported is True
    assert connection.is_push_supported != False
...
from custom_components.climate_ip.const import PROTOCOL_2878_POWER_ID, PROTOCOL_2878_VALUE_ON
...
async def test_samsung_2878_process_command_queue(connection):
    ...
    with patch.object(connection, '_write_data', new_callable=AsyncMock) as mock_write:
        ...
        assert getattr(connection._pending_future, "_command_debug") == '<Request Type="Test"></Request>\n'
...
def test_track_task_safe(connection):
    ...
    with patch("asyncio.create_task") as mock_create:
        ...
        mock_task.add_done_callback.assert_called_once_with(connection._background_tasks.discard)
...
async def test_read_full_response(connection):
    """Mata los mutantes de lectura de red en un solo chunk."""
    connection._reader = MagicMock()
    connection._reader.at_eof.return_value = True
    assert await connection._read_full_response() is None

    connection._reader.at_eof.return_value = False
    connection._reader.read = AsyncMock(return_value=b'<?xml version="1.0"?><Response>DPLUG-1.6</Response>')
    response = await connection._read_full_response()
    assert "DPLUG-1.6" in response

    connection._reader.read = AsyncMock(side_effect=TimeoutError)
    assert await connection._read_full_response() is None
    
    connection._reader.read = AsyncMock(side_effect=ConnectionResetError("Reset"))
    with patch.object(connection, '_close_connection', new_callable=AsyncMock) as mock_close:
        assert await connection._read_full_response() is None
...
async def test_establish_connection_network_refused(connection):
    ...
    with patch("socket.socket"), \
         ...
         patch("custom_components.climate_ip.samsung_2878.async_create_samsung_ssl_context", new_callable=AsyncMock):
...
async def test_process_read_queue_fragmented(connection):
    ...
    with patch.object(connection, '_parse_and_update_state', new_callable=AsyncMock) as mock_parse:
...
async def test_establish_connection_success(connection):
    ...
    with patch("socket.socket") as mock_socket, \
         ...
         patch("custom_components.climate_ip.samsung_2878.async_create_samsung_ssl_context", new_callable=AsyncMock) as mock_ssl_ctx:
        ...
        with patch.object(connection, "_read_full_response", new_callable=AsyncMock) as mock_read_resp, \
             patch.object(connection, "_write_data", new_callable=AsyncMock) as mock_write, \
             patch.object(connection, "_parse_and_update_state", new_callable=AsyncMock) as mock_parse:
...
async def test_establish_connection_ssl_cache_and_ciphers(connection):
    ...
    with patch("socket.socket") as mock_socket, \
        ...
        with patch.object(connection, "_read_full_response", new_callable=AsyncMock) as mock_read, \
             patch.object(connection, "_write_data", new_callable=AsyncMock), \
             patch.object(connection, "_parse_and_update_state", new_callable=AsyncMock) as mock_parse, \
             patch.object(connection, "_post_connect_status_request", new_callable=AsyncMock):
            ...
            mock_sock_inst.setsockopt.assert_any_call(socket.SOL_SOCKET, socket.SO_KEEPALIVE, 1)
...
async def test_handle_reconnection_backoff(connection):
    ...
    with patch("custom_components.climate_ip.samsung_2878.async_check_network_reachability", new_callable=AsyncMock) as mock_ping, \
         patch.object(connection, "_establish_connection_and_handshake", new_callable=AsyncMock) as mock_handshake, \
...
async def test_connection_manager_loop_logic(connection):
    ...
    with patch.object(connection, "handle_reconnection", new_callable=AsyncMock) as mock_reconn, \
...
def test_load_from_yaml_missing_params():
    ...
    from custom_components.climate_ip.samsung_2878 import ConnectionSamsung2878
    conn = ConnectionSamsung2878({"host": "192.168.1.10"}, MagicMock(), hass=MagicMock())
...
async def test_parse_state_multiple_docs(connection):
    ...
    async def mock_add_exec(func, *args, **kwargs):
        return func(*args, **kwargs)
    connection._controller.hass.async_add_executor_job = AsyncMock(side_effect=mock_add_exec)
...
async def test_establish_connection_cipher_iteration_and_issue_clear(connection):
    ...
    with patch("socket.socket"), \
         ...
         patch("asyncio.open_connection", new_callable=AsyncMock) as mock_open_conn, \
         patch("custom_components.climate_ip.samsung_2878.async_create_samsung_ssl_context", new_callable=AsyncMock), \
         patch("custom_components.climate_ip.samsung_2878.async_delete_issue") as mock_delete:
...
async def test_connection_manager_full_coverage(connection):
    ...
    with patch.object(connection, "_process_command_queue", new_callable=AsyncMock) as mock_cmd, \
         patch.object(connection, "_process_read_queue", new_callable=AsyncMock) as mock_read_q:
...
async def test_handle_reconnection_success_path(connection):
    ...
    with patch("custom_components.climate_ip.samsung_2878.async_check_network_reachability", new_callable=AsyncMock, return_value=True), \
         patch.object(connection, "_establish_connection_and_handshake", new_callable=AsyncMock, return_value=True):
...
async def test_establish_connection_callbacks_and_generic_catch(connection):
    ...
    with patch("socket.socket"), patch("asyncio.get_running_loop") as mock_loop, \
         patch("custom_components.climate_ip.samsung_2878.async_create_samsung_ssl_context", new_callable=AsyncMock):
         
        mock_loop.return_value.sock_connect = AsyncMock(side_effect=[RuntimeError("Weird network issue"), None])
...
async def test_establish_connection_deep_mutants(connection):
    ...
    with patch("socket.socket"), patch("asyncio.get_running_loop") as mock_loop, \
        ...
        with patch.object(connection, "_read_full_response", new_callable=AsyncMock) as mock_read, \
            ...
            with pytest.raises(CannotConnect, match="Did not receive expected initial message"):
                await connection._establish_connection_and_handshake()
            
            # CASO 2: Plantilla vacía
            connection._connection_init_template = None
            mock_read.return_value = "DPLUG-1.6"
            with pytest.raises(CannotConnect, match="Connection initialization template is missing"):
                await connection._establish_connection_and_handshake()
            ...
            from custom_components.climate_ip.exceptions import AuthError
            with pytest.raises(AuthError, match="Authentication failed: No response|Authentication failed"):
                await connection._establish_connection_and_handshake()
...
async def test_read_full_response_buffer_decode(connection):
    ...
    async def mock_read(*args, **kwargs):
        ...
        call_count += 1
        if call_count == 1:
            return b"<PartialData>"
        elif call_count == 2:
            return b""
        else:
            # El escudo anti-mutantes: Si mutmut rompe el break, explota aquí en lugar de colgarse
            raise RuntimeError("Infinite loop prevented in AsyncMock")
    
    connection._reader.read = mock_read
    
    with patch.object(connection, '_close_connection', new_callable=AsyncMock) as mock_close:
        try:
            res = await connection._read_full_response()
            assert res == "<PartialData>"
            mock_close.assert_called_once()
        except RuntimeError as e:
            if "Infinite loop prevented" in str(e):
                pass # Mutante aniquilado por nuestro escudo
            else:
                raise
...
def test_start_listening_strict_assignments(connection):
    ...
    with patch("asyncio.create_task") as mock_create, \
         patch.object(connection, "_connection_manager") as mock_manager:
...
async def test_read_full_response_decode_errors(connection):
    ...
    sig = inspect.signature(connection._read_full_response)
    assert sig.parameters['timeout'].default == 10.0
    
    connection._reader = MagicMock()
    connection._reader.at_eof.return_value = False
    
    # Inyectamos un byte inválido utf-8 (0xFF) y comprobamos que el 'errors="ignore"' sobrevive
    # Si mutmut cambia 'ignore' por 'strict' o lo borra, el decode() levantará UnicodeDecodeError y el test fallará
    connection._reader.read = AsyncMock(side_effect=[b"<Update>BadByte\xff</Update>", b""])
    
    res = await connection._read_full_response()
    # Si funcionó el ignore, el 0xFF se borró y quedó el string limpio
    assert res == "<Update>BadByte</Update>"
...
def test_execute_sync_not_implemented(connection):
    ...
    with pytest.raises(NotImplementedError, match="^ConnectionSamsung2878 is async-native\\. Use async_execute\\.$"):
        connection.execute(None, None, None)
...
def test_force_unavailability_if_needed_strict(connection):
    ...
    try:
        ...
        error_msg = str(e)
        if "on_offline_callback" in error_msg or "on_connection_failed_callback" in error_msg:
            pytest.fail(f"¡Mutante aniquilado! Intentó acceder a un callback sin validarlo previamente (hasattr/and roto): {e}")
        else:
...
async def test_read_queue_strict_xml_tags(connection):
    ...
    mock_task = asyncio.Future()
    # Ponemos dos tags juntos. Si usa rfind(), cortará por el segundo y fallará la aserción.
    mock_task.set_result(b"A</Response>B</Response>")
    connection._read_task = mock_task
    
    with patch.object(connection, "_parse_and_update_state", new_callable=AsyncMock) as mock_parse:
        mock_parse.return_value = (False, False, None)
        res = await connection._process_read_queue(b"")
        assert res == b"B</Response>"
        mock_parse.assert_called_once_with("A</Response>")

    # 2. Mata mutantes de strings de Update
    mock_task2 = asyncio.Future()
    mock_task2.set_result(b"X</Update>Y</Update>")
...
async def test_parse_state_strict_dicts_and_logic(connection):
    ...
    is_resp, is_upd, parsed = await connection._parse_and_update_state(xml_missing_state)
...
async def test_connection_manager_strict_buffer(connection):
    ...
    with patch("asyncio.create_task", side_effect=mock_create_task), \
         ...
         patch.object(connection, "_process_read_queue", new_callable=AsyncMock) as mock_process_read, \
         patch("custom_components.climate_ip.samsung_2878.asyncio.sleep", new_callable=AsyncMock), \
         patch.object(connection, "_close_connection", new_callable=AsyncMock):
...
async def test_post_connect_status_request_strict_put(connection):
    ...
    with patch("custom_components.climate_ip.samsung_2878.asyncio.sleep", new_callable=AsyncMock), \
         patch("custom_components.climate_ip.samsung_2878.asyncio.timeout", side_effect=TimeoutError):
...
async def test_handle_reconnection_strict_sleep_and_kwargs(connection):
    ...
    with patch("custom_components.climate_ip.samsung_2878.async_check_network_reachability", return_value=True), \
         patch.object(connection, "_establish_connection_and_handshake", side_effect=CannotConnect("Test error")), \
         patch.object(connection, "_close_connection", new_callable=AsyncMock), \
         patch("custom_components.climate_ip.samsung_2878.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
...
def test_update_configuration_strict_dicts(connection):
    ...
    from custom_components.climate_ip.samsung_2878 import CONF_DUID
    
    # Simulamos un config dict que registre las llamadas exactas a .get()
    hass_config = MagicMock()
    hass_config.get.side_effect = lambda k, default=None: default if k == CONF_PORT else f"val_{k}"
    
    with patch("custom_components.climate_ip.samsung_2878.format_placeholders") as mock_format:
...
def test_check_and_create_repair_issue_strict_getattr(connection):
    ...
    with patch("custom_components.climate_ip.samsung_2878.async_create_issue") as mock_issue:
        try:
            connection._check_and_create_repair_issue()
...
async def test_read_full_response_strict_decoding(connection):
    """Mata mutantes de decodificación y buffers iterativos en _read_full_response."""
    from unittest.mock import AsyncMock, MagicMock, patch
    ...
    # Creamos un Context Manager Asíncrono de mentira para aislar el test del reloj real
    class DummyTimeout:
        async def __aenter__(self): return self
        async def __aexit__(self, exc_type, exc, tb): pass

    mock_timeout = MagicMock(return_value=DummyTimeout())
    
    with patch("custom_components.climate_ip.samsung_2878.asyncio.timeout", mock_timeout):
        # 1. Matar mutante "buffer = chunk" vs "buffer += chunk"
        connection._reader.read.side_effect = [b"A", b"B</Response>", b""]
        res = await connection._read_full_response(timeout=10.0)
        assert res == "AB</Response>"
        # Verificamos que se respetó el parámetro por defecto (10.0)
        mock_timeout.assert_called_with(10.0)
        
        # 2. Matar mutante decode("utf-8", errors="ignore") inyectando un byte corrupto
        connection._reader.read.side_effect = [b"BadByte\xff</Update>", b""]
        res2 = await connection._read_full_response(timeout=10.0)
        assert res2 == "BadByte</Update>"
        
        # 3. Matar mutantes de retornos fantasma en _parse_and_update_state
        is_resp, is_upd, parsed = await connection._parse_and_update_state("")
        assert is_resp is False and is_upd is False and parsed is None
...
def test_update_config_and_yaml_strict_logic(connection):
    ...
    with patch("os.path.dirname", side_effect=mock_dirname):
        connection.update_configuration_from_hass({"preferred_connection": {"cert": "mycert.pem"}})
...
async def test_handle_reconnection_handshake_false(connection):
    ...
    connection._cfg.host = "1.2.3.4"
    with patch("custom_components.climate_ip.samsung_2878.async_check_network_reachability", return_value=False), \
         patch.object(connection, "_close_connection", new_callable=AsyncMock), \
         patch("custom_components.climate_ip.samsung_2878.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
...
async def test_establish_connection_strict_sockets(connection):
    ...
    with patch("socket.socket", return_value=mock_socket_instance) as mock_socket_cls, \
         patch("asyncio.get_running_loop") as mock_loop, \
         patch("asyncio.open_connection", new_callable=AsyncMock) as mock_open_conn, \
         patch("custom_components.climate_ip.samsung_2878.async_create_samsung_ssl_context", new_callable=AsyncMock) as mock_ssl_ctx:
        ...
        mock_writer.get_extra_info.side_effect = lambda k: MagicMock() if k == "ssl_object" else None
        mock_open_conn.return_value = (MagicMock(), mock_writer)
        ...
        with patch.object(connection, "_read_full_response", new_callable=AsyncMock) as mock_read, \
            ...
            mock_loop.return_value.sock_connect.assert_called_once_with(mock_socket_instance, ("10.0.0.1", 2878))
