...
async def test_get_session_keep_alive_no_session(connection_config, mock_logger, mock_hass):
...
async def test_get_session_recreate_closed_local_session(connection_config, mock_logger, mock_hass):
    with patch("os.path.exists", return_value=True):
        conn = ConnectionAiohttp8888(connection_config, mock_logger, mock_hass, None, "192.168.1.100")
        conn._keep_alive = False
        ...
        with patch.object(type(session1), "closed", new_callable=PropertyMock, return_value=True):
            session2 = await conn._get_session()
