...
from .test_token_acquirer_yaml__common import mock_hass, stream_config, listener_config
...
async def test_stream_mode_no_connection(mock_hass, stream_config):
    ...
    with patch.object(acq, "_connect_stream", return_value={"ok": True}):
        ...
        with pytest.raises(TokenAcquisitionError, match="Connection stream not established"):
