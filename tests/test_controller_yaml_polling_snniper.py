...
from custom_components.climate_ip.const import CONF_DEVICE_TYPE, DEVICE_TYPE_SAMSUNG_2878, CONFIG_DEVICE, DEVICE_TYPE_MIM_H03
...
async def test_sniper_build_device_state_from_props_2878_and_options():
    mock_controller = NakedObj(log_prefix="TEST", config={CONF_DEVICE_TYPE: DEVICE_TYPE_SAMSUNG_2878})
    poller = YamlStatePoller(mock_controller)
    
    op_generic = NakedObj(id="custom_prop", value="val")
    poller.controller.loader = NakedObj(state_getter=NakedObj(value={}), operations={"op1": op_generic}, properties={})
    
    with patch.object(poller, "_get_cached_device_key_from_prop", return_value="custom_key") as mock_get_key:
...
def test_sniper_calculate_structured_state_and_rebuild_attrs():
    ...
    poller.controller.loader = NakedObj(is_fully_initialized=True, operations={}, properties={"hvac": prop}, sensors={})
...
async def test_sniper_async_get_status_connection_id():
    ...
    with patch("custom_components.climate_ip.controller_yaml_polling._LOGGER.debug") as mock_debug:
...
async def test_sniper_async_merge_device_state_strict():
    mock_controller = NakedObj(log_prefix="TEST", get_current_state_callback=MagicMock(return_value="valid_state"))
    poller = YamlStatePoller(mock_controller)
    ...
    poller.controller.loader = NakedObj(
        state_getter=NakedObj(value={"base": "state"}),
        _parsed_yaml_cache={}
    )
    ...
    assert poller.async_update_properties_from_state.call_args[1]["current_hass_state"] == "valid_state"
...
async def test_sniper_predict_and_correct_missing_ids_and_shutdown():
    ...
    poller.controller.loader = NakedObj(
        ...
        operations={"op1": op}, properties={"prop1": prop}, connection=NakedObj(stop_listening=AsyncMock(), close=AsyncMock())
    )
    ...
    features, corrections = await poller.async_predict_and_correct_state(NakedObj(), "op1", "val")
...
async def test_sniper_update_properties_cache_and_get_fallbacks():
    ...
    class StrictCache(dict):
        def get(self, key, default=None):
            if key is None: raise KeyError("Mutant detected")
            return super().get(key, default)
            
    poller.controller.loader = NakedObj(
        is_fully_initialized=True,
        _parsed_yaml_cache=StrictCache({"0": {}}), 
        operations={}, properties={}, sensors={}
    )
    poller._rebuild_attributes = MagicMock()
    
    with patch("custom_components.climate_ip.controller_yaml_polling._LOGGER.exception") as mock_exc:
    ...
    poller.controller.loader._parsed_yaml_cache["0"][CONFIG_DEVICE] = {"identifiers": {"dummy": "val"}}
    
    with patch("custom_components.climate_ip.controller_yaml_polling.get_value_by_path", side_effect=[[{"id": "0"}], "0"]) as mock_get_path:
        await poller.async_update_properties_from_state({"dummy": "data"})
...
async def test_sniper_update_properties_delegations():
    ...
    prop = NakedObj(id="prop1", name="PropName", get_connection=MagicMock(return_value=None))
    prop.convert_hass_to_dev = MagicMock(return_value="dev_val")
    prop.async_update_state = AsyncMock()
    
    op = NakedObj(id="op1", value="a", values=["a", "b"], get_connection=MagicMock(return_value=None))
    op.is_valid = MagicMock(return_value=True)
    
    poller.controller.loader = NakedObj(
        is_fully_initialized=True,
        operations={"op1": op}, properties={"prop1": prop}, sensors={},
        _parsed_yaml_cache={}
    )
    poller._rebuild_attributes = MagicMock()
    ...
    with patch.object(poller, "_get_cached_device_key_from_prop", return_value="prop1_key"):
...
async def test_sniper_async_update_state_token_and_debug():
    mock_controller = NakedObj(log_prefix="TEST", token="OLD_TOKEN", debug=True, config={CONF_DEVICE_TYPE: "REST"})
    poller = YamlStatePoller(mock_controller)
...
async def test_sniper_discovery_empty_or_invalid_devices():
    ...
    mock_controller = NakedObj(log_prefix="TEST", config={CONF_DEVICE_TYPE: DEVICE_TYPE_MIM_H03}, ip_address="1.1.1.1", device_id="0")
    poller = YamlStatePoller(mock_controller)
    ...
    poller.controller.loader = NakedObj(
        ...
        operations={}, sensors={}, properties={}
    )
    poller.controller.loader.state_getter.async_update_state = AsyncMock(return_value={"ok": True})
    poller._consecutive_connection_errors = 0
    poller.async_update_properties_from_state = AsyncMock()
    
    poller.controller.loader._parsed_yaml_cache = {
        "0": {CONFIG_DEVICE: {"identifiers": {"path_to_devices": ["List"], "id": ["id"]}}}
    }
    
    bad_devices = [[{"id": "0"}], [{"id": "valid", "NoModeHere": True}], []]
    
    with patch("custom_components.climate_ip.controller_yaml_polling.get_value_by_path", side_effect=bad_devices), \
         patch("custom_components.climate_ip.controller_yaml_polling.async_check_network_reachability", return_value=True):
...
async def test_sniper_build_device_state_fails_on_missing_value():
    ...
    poller.controller.loader = NakedObj(state_getter=NakedObj(), operations={}, properties={})
...
async def test_sniper_build_device_state_success_flow():
    ...
    with patch.object(poller, "_get_cached_device_key_from_prop", return_value="target_key") as mock_get_key:
...
async def test_sniper_async_merge_device_state_protected_value():
    ...
    poller.controller.loader = NakedObj(state_getter=st_getter, is_fully_initialized=True, operations={}, properties={}, sensors={})
    poller._build_device_state_from_hass = AsyncMock(return_value={"base": "state"})
    poller._calculate_structured_state = MagicMock(return_value=NakedObj())
    poller._evict_invalidated_pending_updates = MagicMock()
    poller.async_update_properties_from_state = AsyncMock()
...
async def test_sniper_merge_device_state_protected_value_mutation():
    ...
    assert st_getter._value == {"base": "state", "new": "data"}, "Mutante L899: Corrupción en la escritura de _value"
...
async def test_sniper_update_properties_pending_and_is_valid_mutations():
    ...
    with patch.object(poller, "_get_cached_device_key_from_prop", return_value="prop1_key") as mock_get_key:
        ...
        assert base_state.get("prop1_key") == "dev_pending_val", "Mutante L534: Falló la inyección por pasar None al key finder"
        ...
        assert args[0] == base_state, "Mutante L567: is_valid evaluado ciegamente con None"
...
async def test_sniper_async_update_state_network_and_discovery_strictness():
    ...
    poller.controller.loader = NakedObj(
        state_getter=NakedObj(value={"dummy": "state"}), # <-- CORRECCIÓN: El atributo exigido por la ley marcial
        is_fully_initialized=False, 
        async_finish_initialization=AsyncMock(),
        operations={},
        sensors={},
    ...
    with patch("custom_components.climate_ip.controller_yaml_polling.async_check_network_reachability", return_value=True):
        ...
        poller.controller.loader.state_getter.async_update_state = AsyncMock(side_effect=RequestException("Simple Error"))
        
        res = await poller.async_update_state()
        assert res == {"cached": True}, "Mutante L328 sobrevivió: no respetó la barrera de <= 2 errores"
        ...
        poller._cached_device_state = {"cached": True}
        poller.controller.loader.state_getter.async_update_state = AsyncMock(side_effect=RequestException("HTTP Error 500: Timeout on backend   "))
        
        with pytest.raises(UpdateFailed) as exc_info:
            await poller.async_update_state()
            
        assert poller._consecutive_connection_errors == 3
        assert "Device unreachable: Timeout on backend" in str(exc_info.value), "Mutante L344/L345 sobrevivió: la razón del error fue corrupta"

    # --- PRUEBA 3: Configuración estricta (Sin Cambios) ---
    class StrictConfig(dict):
        def get(self, key, default=None):
            if key is None: raise KeyError("Mutant L366")
            return super().get(key, default)
    ...
    with patch("custom_components.climate_ip.controller_yaml_polling.get_value_by_path", side_effect=[[{"id": "new_id", "Mode": True}], "new_id"]) as mock_path, \
         patch("custom_components.climate_ip.controller_yaml_polling.async_check_network_reachability", return_value=True), \
         patch("custom_components.climate_ip.controller_yaml_polling._LOGGER.info") as mock_info:
...
async def test_update_properties_private_value_pending():
    ...
    prop_private = NakedObj(id="hidden_prop", _value="old_val", async_update_state=lambda *args: None)
