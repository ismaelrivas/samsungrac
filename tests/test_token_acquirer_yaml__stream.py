...
async def test_connect_stream_missing_ciphers_raises_cannot_connect(mock_hass, stream_config):
...
async def test_connect_stream_strategy_default_verify_mode(mock_hass, stream_config):
    ...
    with (
        ...
        patch("custom_components.climate_ip.token_acquirer_yaml.async_create_samsung_ssl_context", return_value=MagicMock()) as mock_ssl,
        patch("asyncio.open_connection", return_value=(mock_reader, mock_writer)),
        patch("custom_components.climate_ip.token_acquirer_yaml.asyncio.timeout", return_value=make_mock_timeout_cm()),
    ):
        await acq._connect_stream()
...
async def test_connect_stream_user_cert_relative_preserves_user_cert_path(mock_hass, stream_config):
    """Test that __user_cert__ strategy restores original user_cert_path even if relative."""
    stream_config["tls_config"]["strategies"] = [{"cert": "__user_cert__", "verify_mode": "CERT_NONE"}]
    acq = GenericYamlTokenAcquirer(mock_hass, "192.168.1.100", stream_config, cert_path="relative_user.pem")
    mock_reader = AsyncMock()
    mock_writer = AsyncMock()
...
async def test_initiate_pairing_stream_timeout_and_custom_buffer(mock_hass, stream_config):
...
async def test_initiate_pairing_stream_match_and_fallback_extraction(mock_hass, stream_config):
    """Test that match and fallback_match are extracted using exact default empty strings."""
    mock_succ_cfg = MagicMock()
    mock_succ_cfg.get.side_effect = lambda k, default: 'Status="Ready"' if k == "match" else ""
    ...
    with (
        ...
        config = await acq.async_initiate_pairing()
        mock_succ_cfg.get.assert_has_calls([call("match", ""), call("fallback_match", "")])
        assert config == {"cert": None, "verify_mode": 0}
...
async def test_initiate_pairing_stream_match_not_found_no_fallback(mock_hass, stream_config):
    """Test initiate pairing raises when match is not found and no fallback is specified."""
    stream_config["request_pairing"]["success_template"] = {"match": "StrictMatchRequired"}
    acq = GenericYamlTokenAcquirer(mock_hass, "192.168.1.100", stream_config, cert_path=None)

    mock_reader = AsyncMock()
    mock_writer = AsyncMock()
    mock_writer.drain = AsyncMock()
    mock_reader.read.return_value = b'<Response Type="GetToken" Status="DifferentStatus"/>'
    acq._reader = mock_reader
    acq._writer = mock_writer
...
async def test_initiate_pairing_stream_fallback_match_success(mock_hass, stream_config):
    ...
    mock_writer = AsyncMock()
    mock_writer.drain = AsyncMock()
    mock_reader.read.return_value = b'<Response Type="GetToken" SecondaryAccepted="true"/>'
    acq._reader = mock_reader
    acq._writer = mock_writer
...
async def test_wait_for_token_custom_timeout_and_custom_buffer(mock_hass, stream_config):
    ...
    mock_reader = AsyncMock()
    mock_reader.read.return_value = b'<Update Type="Authenticate" Status="Success" Token="custom_99_token"/>'
    acq._reader = mock_reader
...
async def test_wait_for_token_explicit_utf8_decoding(mock_hass, stream_config):
    ...
    mock_data = MagicMock(spec=bytes)
    mock_data.__len__.return_value = 50
    mock_data.decode.return_value = '<Update Type="Authenticate" Status="Success" Token="utf8_stream_tok"/>'
...
async def test_wait_for_token_stream_missing_extract_template(mock_hass, stream_config):
    ...
    mock_reader = AsyncMock()
    mock_reader.read.return_value = b'<Update Type="Authenticate" Status="Success" Token="token"/>'
    acq._reader = mock_reader
