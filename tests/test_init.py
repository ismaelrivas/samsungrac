...
async def test_unload_while_connecting(hass: HomeAssistant) -> None:
    ...
    with patch.object(conn, "_close_connection", new_callable=AsyncMock) as mock_close:
        ...
        with patch.object(mock_task, "cancel", side_effect=mock_cancel_side_effect) as mock_cancel_call:
...
async def test_migration_v1_validates_mac_strictly(hass: HomeAssistant) -> None:
    ...
    mock_entry.data = {"ip_address": "192.168.1.100", "token": "valid", "mac": ["invalid"]}
...
async def test_setup_entry_instantiates_controller_strictly(hass: HomeAssistant) -> None:
    ...
    with patch("custom_components.climate_ip.YamlController") as mock_yaml_class:
        ...
        with patch("custom_components.climate_ip.async_get_clientsession") as mock_get_session, \
        ...
        assert "config" in kwargs and kwargs["config"] is not None, "The config argument was omitted or is None"
        assert "logger" in kwargs and kwargs["logger"] is not None, "The logger argument was omitted or is None"
        assert "hass" in kwargs and kwargs["hass"] is hass, "The hass argument was not passed correctly"
        assert "session" in kwargs and kwargs["session"] is not None, "The session argument was omitted or is None"
...
async def test_setup_entry_multi_device_branch_and_unique_id_logic(hass: HomeAssistant) -> None:
    ...
    with patch("custom_components.climate_ip.YamlController") as mock_yaml_class, \
         ...
         patch.object(hass.config_entries, "async_forward_entry_setups") as mock_forward, \
         patch("custom_components.climate_ip.SamsungClimateCoordinator") as mock_coordinator_class:
        ...
        assert mock_yaml_class.call_count == 3, "The loop was prematurely broken or failed to filter ID 0."
        ...
        assert config_a.get("unique_id") == "uuid-111_1", "Failed to generate compound unique_id."
        ...
        assert config_b.get("unique_id") == "uuid-222_2", "Duplicated the suffix or altered existing unique_id."

        # Validate Zone C (Fallback to entry.unique_id)
        config_c = call_args_list[2].kwargs["config"]
        assert config_c.get(CONF_DEVICE_ID) == "3"
        assert config_c.get("unique_id") == "parent_entry_mac_3", "Fallback to entry unique_id not applied correctly."
        
        # Mutant 71: Verify coordinators were saved in runtime_data
        assert isinstance(mock_entry.runtime_data, dict), "The coordinators dictionary was not saved in runtime_data"
        ...
        assert all(c is not None for c in mock_entry.runtime_data.values()), "None was assigned to the coordinators dictionary instead of the instance"
...
async def test_setup_entry_total_initialization_failure(hass: HomeAssistant) -> None:
    ...
    with patch("custom_components.climate_ip.YamlController") as mock_yaml_class, \
        ...
        assert result_standalone is False, "Standalone did not return False on a total failure"
        ...
        assert result_multi is False, "Multi-device did not return False on a total failure"
...
async def test_setup_entry_multi_device_partial_failure(hass: HomeAssistant) -> None:
    ...
    with patch("custom_components.climate_ip.YamlController") as mock_yaml_class, \
        ...
        assert mock_coord_class.call_count == 1, "The loop executed break instead of continue."
...
async def test_setup_entry_coordinator_instantiation_strict(hass: HomeAssistant) -> None:
    ...
    mock_entry.data = {"ip_address": "192.168.1.100", CONF_DEVICES: [{"id": "1", "name": "Zone A"}]}
    mock_entry.options = {}
    mock_entry.version = 2
    ...
    with patch("custom_components.climate_ip.YamlController") as mock_yaml_class, \
        ...
        assert args[1] is mock_instance, "The 'controller' argument is None or incorrect"
        assert args[2] is mock_entry, "The 'entry' argument is incorrect"
        
        # LETHAL ASSERTIONS (Mutants 61, 62, 66, 67)
        assert "device_info" in kwargs and kwargs["device_info"] is not None, "The 'device_info' argument was omitted"
        assert "parent_unique_id" in kwargs and kwargs["parent_unique_id"] == "parent_mac", "The 'parent_unique_id' argument is incorrect"
