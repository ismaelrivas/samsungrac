...
async def test_mutant_deepcopy_async_get_status():
    ...
    res["nested"]["k"] = "hacked"
    assert poller.controller.loader.state_getter.value["nested"]["k"] == "safe", "Deepcopy mutated to copy!"
    ...
    assert poller._pure_network_state["nested_pure"]["k"] == "safe", "Merge deepcopy mutated!"
...
async def test_mutant_async_update_state_available_consecutive():
    ...
    assert "Device unreachable: ExpectedReason" in str(exc.value), "Split index mutated!"
...
async def test_mutant_predict_and_correct_logic_flips():
    ...
    poller.async_update_properties_from_state = AsyncMock(return_value={"other_prop": "v2"})
...
async def test_mutant_async_update_state_available_flip():
    ...
    poller._consecutive_connection_errors = 1
    poller.controller.loader.state_getter.async_update_state = AsyncMock(side_effect=CannotConnect("x"))
...
async def test_mutant_predict_action_removals():
    ...
    assert fake_memory_state["power_node"] == "On", "Mutant survived: _inject_value_into_state was removed!"
