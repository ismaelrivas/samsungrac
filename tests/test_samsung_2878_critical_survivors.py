...
from custom_components.climate_ip.samsung_2878 import ConnectionConfig, ConnectionSamsung2878
...
async def test_read_full_response_success_and_timeout_args(connection):
    ...
    assert res3 == "<Response>OK</Response>" # Si mutó, esto fallará porque devolvería None o string vacío
...
async def test_read_full_response_timeout_exception(connection):
    ...
    connection._reader.read = AsyncMock(side_effect=[b"Part1", asyncio.IncompleteReadError(b"Part1", None)])
...
async def test_read_full_response_logic_and_concat(connection):
    ...
    connection._reader.read = AsyncMock(side_effect=[b"Part1</Response>", b"ExtraChunk", b""])
    res = await connection._read_full_response()
    assert res == "Part1</Response>" # Si lee ExtraChunk, fallará
    
    # 2. Test para </Update>
    connection._reader.read = AsyncMock(side_effect=[b"Part1</Update>", b"ExtraChunk", b""])
    res = await connection._read_full_response()
...
async def test_read_full_response_logic_dplug_only(connection):
    ...
    connection._reader.read = AsyncMock(side_effect=[dplug_bytes + b" NO_END", b"ExtraChunk", b""])
    res = await connection._read_full_response()
...
async def test_connection_manager_critical_survivors(connection):
    ...
    with patch("custom_components.climate_ip.samsung_2878.asyncio.sleep", AsyncMock()):
        ...
        with patch.object(connection, "handle_reconnection", new_callable=AsyncMock) as mock_recon:
            class KillMutant(BaseException): pass
            ...
            mock_recon.side_effect = [True, KillMutant("El mutante 'continue' ha sobrevivido!")]
            try:
